...
class ActualsImportService {
    ...
    async commitImport(rows, userId) {
        return await prisma.$transaction(async (tx) => {
            ...
            // Fetch all currency rates first
            ...
            for (const row of rows) {
                ...
                } else if (rateMap[row.currency]) {
                    convertedAmount = row.amount * rateMap[row.currency];
                } else {
                    // Fallback to null or error? For now leave null if no rate found, 
                    // or assume 1:1 if user wants. But usually we want to know it failed.
                    // Given strict mode, maybe log it.
                    // Ideally we should have validated this in validateRow but we didn't have async access there effortlessly.
                    // We'll leave it as null, which might mean "Pending Conversion" in UI.
