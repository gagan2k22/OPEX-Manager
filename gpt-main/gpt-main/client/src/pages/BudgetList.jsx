...
const BudgetList = () => {
    ...
    const [selectedMonths, setSelectedMonths] = useState([]); // Array of strings (e.g., 'Jan' or 'Jan 2025')
    const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

    // Dynamic Month Options
    const monthOptions = useMemo(() => {
        if (selectedYears.length > 1) {
            return selectedYears.flatMap(year => monthNames.map(m => `${m} ${year}`));
        }
        return monthNames;
    }, [selectedYears]);

    // Reset months when switching between single and multi-year modes
    useEffect(() => {
        setSelectedMonths([]);
    }, [selectedYears.length > 1]); // Dependency on mode switch boolean
    ...
    const processedBudgets = useMemo(() => {
        return budgets.map(b => {
            // Clone
            const newB = { ...b };
            // Recalculate if months selected
            // Recalculate based on year-specific month selection
            ['FY25', 'FY26', 'FY27'].forEach(fy => {
                const yearNum = '20' + fy.slice(2);
                const isMultiYear = selectedYears.length > 1;
                const yKey = fy.toLowerCase();

                let relevantMonths = [];
                const globalSelectionExists = selectedMonths.length > 0;

                if (!globalSelectionExists) {
                    // No filters at all -> Include All
                    return; // No change needed, keeps original values
                }

                if (isMultiYear) {
                    // Filter: "Jan 2025" -> matches 2025 only
                    relevantMonths = selectedMonths
                        .filter(m => m.endsWith(` ${yearNum}`))
                        .map(m => m.split(' ')[0]);
                } else {
                    // Single year mode: "Jan" applies to the single selected year
                    // (But iterating FY25/26/27... only the matching year matters)
                    if (selectedYears.includes(yearNum)) {
                        relevantMonths = selectedMonths;
                    }
                }

                // Apply Calculation
                // If global selection exists, we STRICTLY filter. 
                // If relevantMonths is empty for this year (and global selection exists), value is 0.
                if (b.fiscal_year_name === fy) {
                    if (relevantMonths.length > 0) {
                        newB[`${yKey}_budget`] = b.monthly_data
                            .filter(m => relevantMonths.includes(m.month))
                            .reduce((s, x) => s + x.amount, 0);

                        newB[`${yKey}_actuals`] = b.actual_items
                            .filter(m => relevantMonths.includes(m.month))
                            .reduce((s, x) => s + x.amount, 0);
                    } else {
                        // Global selection exists but nothing for this year -> 0
                        newB[`${yKey}_budget`] = 0;
                        newB[`${yKey}_actuals`] = 0;
                    }
                } else {
                    newB[`${yKey}_budget`] = 0;
                    newB[`${yKey}_actuals`] = 0;
    ...
    }, [budgets, selectedMonths, selectedYears]);
    ...
    const handleExport = useCallback((format) => {
        try {
            ...
            const filename = `Budget_Tracker_${timestamp}.${extension}`;

            // Save file
            XLSX.writeFile(wb, filename);
            showSnackbar(`Budget data exported as ${format.toUpperCase()} successfully!`, 'success');
        } catch (error) {
            console.error('Error exporting to Excel:', error);
            showSnackbar('Error exporting data', 'error');
        }
    }, [filteredBudgets, selectedYears, formatDate, showSnackbar]);
            ...
            <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                ...
                <FormControl sx={{ m: 1, width: 250 }} size="small">
                    ...
                    <Select
                        ...
                        renderValue={(selected) => selected.length === 0 ? 'All Months' : selected.join(', ')}
                ...
                <Typography variant="caption" sx={{ color: 'text.secondary', fontStyle: 'italic' }}>
                    Select Years and Months to filter data.
                </Typography>
            </Box>
                                ...
                                {selectedYears.map(year => {
                                    ...
                                    return (
                                        <React.Fragment key={year}>
                                            <TableCell align="right" sx={{ ...excelTableStyles.headerCell, minWidth: '100px' }}>{fy} Budget</TableCell>
                                            <TableCell align="right" sx={{ ...excelTableStyles.headerCell, minWidth: '100px' }}>{fy} Actuals</TableCell>
                                            <TableCell align="right" sx={{ ...excelTableStyles.headerCell, minWidth: '100px', bgcolor: '#e3f2fd' }}>{fy} Total (UID)</TableCell>
                                            <TableCell align="right" sx={{ ...excelTableStyles.headerCell, minWidth: '100px', bgcolor: '#e3f2fd' }}>{fy} Actuals (UID)</TableCell>
                                        </React.Fragment>
                                    );
                                })}
                        ...
                        <TableBody>
                            {filteredBudgets.map((row) => (
                                <TableRow key={row.id} sx={excelTableStyles.tableRow}>
                                    ...
                                    {selectedYears.map(year => {
                                        const yearNum = year.slice(2).toLowerCase();
                                        return (
                                            <React.Fragment key={year}>
                                                <TableCell sx={{ ...excelTableStyles.dataCell, ...excelTableStyles.numericCell }}>
                                                    {formatCurrency(row[`fy${yearNum}_budget`])}
                                                </TableCell>
                                                <TableCell sx={{ ...excelTableStyles.dataCell, ...excelTableStyles.numericCell }}>
                                                    {formatCurrency(row[`fy${yearNum}_actuals`])}
                                                </TableCell>
                                                <TableCell sx={{ ...excelTableStyles.dataCell, ...excelTableStyles.numericCell, bgcolor: '#e3f2fd' }}>
                                                    {formatCurrency(row[`uid_total_fy${yearNum}_budget`])}
                                                </TableCell>
                                                <TableCell sx={{ ...excelTableStyles.dataCell, ...excelTableStyles.numericCell, bgcolor: '#e3f2fd' }}>
                                                    {formatCurrency(row[`uid_total_fy${yearNum}_actuals`])}
                                                </TableCell>
                                            </React.Fragment>
                                        );
                                    })}
