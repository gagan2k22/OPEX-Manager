...
const ActualBOA = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/actual-boa', {
                headers: {
            ...
            console.error('Error fetching Actual BOA data:', err);
    ...
    const handleSave = async () => {
        try {
            ...
            for (const update of updates) {
                const response = await fetch(`/api/actual-boa/${update.id}`, {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json'
    ...
    const handleSeed = async () => {
        try {
            const response = await fetch('/api/actual-boa/seed', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const handleExport = (format) => {
        try {
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Actual BOA');

            const timestamp = new Date().toISOString().split('T')[0];
            const extension = format === 'csv' ? 'csv' : 'xlsx';
            const filename = `Actual_BOA_${activeTab === 0 ? 'Values' : 'Percentage'}_${timestamp}.${extension}`;
    ...
    return (
        <Box sx={{ ...pageContainerStyles, ...pageTransitionStyles }}>
            <Box sx={pageHeaderStyles}>
                <Typography sx={pageTitleStyles}>Actual BOA</Typography>
