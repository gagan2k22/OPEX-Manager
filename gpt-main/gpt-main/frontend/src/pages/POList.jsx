...
const POList = () => {
    ...
    const processRowUpdate = async (newRow, oldRow) => {
        try {
            ...
            if (newRow.poDate && typeof newRow.poDate === 'object') newRow.poDate = newRow.poDate.toISOString();
            if (newRow.prDate && typeof newRow.prDate === 'object') newRow.prDate = newRow.prDate.toISOString();

            await api.put(`/pos/${newRow.id}`, newRow);
            setPOs(prev => prev.map(p => p.id === newRow.id ? newRow : p));
    ...
    const columns = useMemo(() => [
        ...
        { field: 'prNumber', headerName: 'PR Number', width: 130, editable: isAdmin },
        { field: 'prDate', headerName: 'PR Date', width: 110, valueFormatter: (value) => formatDate(value), type: 'date', valueGetter: (value) => value ? new Date(value) : null, editable: isAdmin },
        {
            field: 'prAmount',
        ...
        { field: 'currency', headerName: 'Currency', width: 80, editable: isAdmin },
        { field: 'poNumber', headerName: 'PO Number', width: 130, editable: isAdmin },
        { field: 'poDate', headerName: 'PO Date', width: 110, valueFormatter: (value) => formatDate(value), type: 'date', valueGetter: (value) => value ? new Date(value) : null, editable: isAdmin },
        ...
        },
        {
            ...
            width: 120,
            valueGetter: (value, row) => (row?.commonCurrencyValue || row?.poValue) / 100000,
    ...
    const filteredRows = useMemo(() => {
        ...
        return pos.filter(row => {
            return columns.some(col => {
                ...
                const params = { value: cellValue, row };

                if (col.valueGetter) {
                    cellValue = col.valueGetter(cellValue, row);
                    params.value = cellValue;
                }
                ...
                if (col.valueFormatter) {
                    ...
                    displayValue = cellValue.toLocaleDateString('en-GB');
